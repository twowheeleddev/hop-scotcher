...
--[[ ========= MOCK VLC OBJECT FOR TESTING ========= ]]
-- Mock the `vlc` object for standalone development and testing
if not vlc then
    print("Running in standalone mode. Mocking `vlc` object for testing.")
    vlc = {
        msg = {
            info = function(msg) print("INFO:", msg) end,
            err = function(msg) print("ERROR:", msg) end
        },
        var = {
            get = function(_, var_name)
                if var_name == "time" then
                    return 120 -- Mock current playback time (2 minutes).
                end
                return nil
            end
        },
        object = {
            input = function()
                return true -- Mock input object to simulate an active media.
            end
        },
        dialog = function(name)
            print("Mock dialog created with name:", name)
            return {
                add_label = function(...)
                    print("Mock add_label:", ...)
                end,
                add_button = function(...)
                    print("Mock add_button:", ...)
                end,
                add_text_input = function(...)
                    print("Mock add_text_input:", ...)
                end,
                set_text = function(_, msg)
                    print("Dialog Message:", msg)
                end,
                delete = function() print("Mock dialog deleted.") end
            }
        end
    }
end
