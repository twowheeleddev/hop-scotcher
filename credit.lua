...
function deactivate() dialog:delete() end
...
function save_profile()
    ...
    if finish_time_input:get_text() == "" then
        finish_time_input:set_text("0")
    end
        ...
        if (child.duration - skip_start - skip_finish) > 0 then
            ...
            if skip_finish > 0 then
                table.insert(options,
                             "stop-time=" .. (child.duration - skip_finish))
            end
        end
...
function load_all_profiles()
    ...
    for _, line in pairs(lines) do
        for name, start_time, finish_time in string.gmatch(line,
                                                           "(.+)=(%d+),(%d+)") do
            table.insert(profiles, {
                name = name,
